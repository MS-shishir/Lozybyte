...
<div class="max-w-4xl mx-auto p-4 md:p-8">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-extrabold text-slate-50 tracking-tight flex items-center gap-2">
            <i class="fa-solid fa-pen-to-square text-indigo-500"></i> Edit FAQ
        </h1>
        <p class="text-slate-400 text-xs mt-1">Update localized question and answer item values.</p>
    </div>
        ...
        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="question_en" 
                           ...
                           value="{{ $faq->question['en'] ?? '' }}" required>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea name="answer_en" rows="5" 
                              class="block w-full px-4 py-2.5 bg-[#0c1322] border border-slate-700/50 focus:border-indigo-500/50 rounded-xl text-slate-200 text-sm outline-none transition-colors resize-y" required>{{ $faq->answer['en'] ?? '' }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="answer_bn" rows="5" 
                              class="block w-full px-4 py-2.5 bg-[#0c1322] border border-slate-700/50 focus:border-indigo-500/50 rounded-xl text-slate-200 text-sm outline-none transition-colors resize-y">{{ $faq->answer['bn'] ?? '' }}</textarea>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-t border-slate-800/60 pt-6 mt-8">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer shrink-0">
                        <input type="checkbox" name="status" class="sr-only peer" value="active" {{ $faq->status ? 'checked' : '' }}>
            ...
            <div class="flex items-center justify-end gap-3 mt-8">
                ...
                <button type="submit" 
                    ...
                    <i class="fa-solid fa-circle-check text-[11px] text-emerald-600"></i> Update FAQ
                </button>
            </div>
